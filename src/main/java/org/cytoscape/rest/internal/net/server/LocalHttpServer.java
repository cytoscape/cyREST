...
/**
 * Creates a server socket and serves http connections from localhost clients
 * only.
 */
public class LocalHttpServer implements Runnable {
	...
	public static final String[] ALLOWED_ORIGINS = new String[] { "http://apps3.nrnb.org", "http://apps.cytoscape.org" };
	...
	/**
	 * Initializes local http server but does not start serving incoming
	 * connections.
	 * 
	 * @param port
	 *            The port on which the http server should receive
	 *            connections--should be greater than 1024 if the server is
	 *            being executed by a non-root user.
	 * @param connectionHandlerExecutor
	 *            executes a connection handler when an incoming socket
	...
	public void run() {
		...
		// Keep servicing incoming connections until this thread is flagged as
		// interrupted
		while (!Thread.interrupted()) { // TODO: **interrupted is deprecated?
			...
			try {
				...
				// System.out.println("Server socket received connection from {}"
	...
	/**
	 * Handles an incoming http connection when it is received by
	 * <code>ConnectionsReceiver</code>.
	 */
	...
	class RequestHandlerDispatcher implements HttpRequestHandler {
		public void handle(final HttpRequest request, final HttpResponse httpResponse, final HttpContext context)
			...
			logger.info("Request received. Method: " + method);
			...
			String originValue = "http://apps.cytoscape.org";
			...
			if (!originFound) {
				if (originValue != null) {
					...
					setHttpResponseToError(httpResponse, HttpStatus.SC_FORBIDDEN, "Access denied", "Origin not found.");
				}
				return;
			}

			if (method.equals("options")) {
				// If the origin is on the allowed list, echo it back as the
				// allowed origin
			...
			if (method.equals("get") || method.equals("head")) {
				...
				logger.info("Number of GET responders: " + getResponders.size());
				for (final GetResponder getResponder : getResponders) {

					// catch any exceptions emitted by the responder so our
					// server thread doesn't prematurely terminate
					// and can send a coherent message back to the user
					try {
						if (getResponder.canRespondTo(url)) {
							logger.info("Responder able to respond. Responding..");
				...
				logger.info("Number of POST responders: " + postResponders.size());
				for (final PostResponder postResponder : postResponders) {
					...
					try {
						if (postResponder.canRespondTo(uri)) {
							logger.info("Found responder. Responding..");
				...
				// none of the http methods are valid, so issue an error to the
				// client
	...
	private static void setHttpResponse(final HttpResponse httpResponse, final int code, final String msg,
	...
	private static void setHttpResponseToError(final HttpResponse httpResponse, final int code, final String title,
			final String msg) throws HttpException {
		setHttpResponse(
				httpResponse,
				code,
				(msg == null ? String.format("<html><body><h1>%s</h1></body></html>", title) : String.format(
						"<html><body><h1>%s</h1><pre>%s</pre></body></html>", title, msg)), "text/html");
	}
	...
	private void setHttpResponseToInternalError(final HttpResponse httpResponse, final Throwable throwable)
			throws HttpException {
